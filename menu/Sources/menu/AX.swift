...
func getMenuItems(
    ...
    for i in children.indices {
        ...
        if children.count == 1, enabled {
            ...
            var modifiers = 0
            var virtualKey = 0
            if let m = getAttribute(element: child, name: kAXMenuItemCmdModifiersAttribute) as? NSNumber {
                modifiers = m.intValue
            }
            if let v = getAttribute(element: child, name: kAXMenuItemCmdVirtualKeyAttribute) as? NSNumber {
                virtualKey = v.intValue
            }
...
private let chineseRegex = try? NSRegularExpression(pattern: "\\p{script=Han}")

func hasChinese(_ text: String) -> Bool {
    guard let regex = chineseRegex else { return false }
